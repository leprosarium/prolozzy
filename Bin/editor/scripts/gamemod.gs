...
// called once per frame so the user can set status bars info
//////////////////////////////////////////////////////////////////////////////////
func MOD_UserUpdate()
{
	if(EdiGet(EDI_TOOL)==0) //PAINT TOOL
	{
		// StatusBar 1
		info="  ";
		id = ToolBrushGet(BRUSH_ID);	
		type = ToolBrushGet(BRUSH_TYPE);
		draw = ToolBrushGet(BRUSH_DRAW);
		drawsz = {"","img","mat","img+mat"};
	
		if(type==0)
		{
			info += "brush";
			if(id!=0) info += "  "+(str)id;
			if(0<draw && draw<=3) info += "  "+drawsz[draw];
			mat = ToolBrushGet(BRUSH_MATERIAL);
			if(mat>=0 && mat<sizeof(g_matname)) info += "  "+g_matname[mat];
		}
		else
		if(type==1)
		{
			info += "object";
			if(id!=0) info += "  "+(str)id;
			if(draw & 1) info += "  "+"visible"; else info += "  "+"hidden"; 
			class = ToolBrushGet(BRUSH_CLASS);
			classname = &(g_brushprop[BRUSH_CLASS][BRUSHPROP_BROWSESEL]); //!careful
			if(class>0 && class<sizeof(*classname)) info += "  "+(*classname)[class];
		}
		SetStatusBar1(info);
		...
		sz += (str "%08X")ToolBrushGet(BRUSH_COLOR)+"  ";
		if(flip==1) sz += "X  ";
		if(flip==2) sz += "Y  ";
		if(flip==3) sz += "XY  ";
		sz += (str)ToolBrushGet(BRUSH_X)+", ";
		sz += (str)ToolBrushGet(BRUSH_Y)+"  ";
		sz += (str)ToolBrushGet(BRUSH_W)+"x";
		sz += (str)ToolBrushGet(BRUSH_H)+"  ";
		SetStatusBar3(sz);
		...
		sz += "\""+MOD_GetRoomInfo(rx,ry)+"\" ";
		sz += "room "+(str)rx+","+(str)ry;
		SetStatusBar4(sz);
	}
	else // EDIT TOOL
	{
		SetStatusBar1("  selected "+(str)EdiGet(EDI_SELECT));
		SetStatusBar3("");
		sz = "";
		rx = EdiGet(EDI_AXEX)/EdiGet(EDI_ROOMW);
		ry = EdiGet(EDI_AXEY)/EdiGet(EDI_ROOMH);
		sz += "\""+MOD_GetRoomInfo(rx,ry)+"\" ";
		sz += "room "+(str)rx+","+(str)ry;
		SetStatusBar4(sz);
	}
	
	SetStatusBar2( g_viewname[g_view]+"  " );
}


//////////////////////////////////////////////////////////////////////////////////
// Map Brush draw callback
// called from code for each brush to adjust some props depending on the view mode
// refers to a temporary ToolBrush, so it don't mess the real brush
// return 0 to refuse draw or 1 to accept
// complexity in this callback can affect scrolling frame time
//////////////////////////////////////////////////////////////////////////////////
func MOD_BrushDraw()
{
	if(g_view==0) return 1;
	if(g_view==1) return (ToolBrushGet(BRUSH_SELECT)!=0);
	if(g_view==2) // game view
	{
		if((ToolBrushGet(BRUSH_DRAW) & 1) == 0) return 0;
		type = ToolBrushGet(BRUSH_TYPE);
		if(type==0)
		{
			return 1;
		...
		if(type==1)
		{
			if(ToolBrushGet(BRUSH_DISABLE)) return 0;
			return 1;
		}
		return 0;
	}
	if(g_view==3) // material view
	{
		if(ToolBrushGet(BRUSH_TYPE)==0)
		{
			if((ToolBrushGet(BRUSH_DRAW) & 2) == 0) return 0;
			mat = ToolBrushGet(BRUSH_MATERIAL);
			if(mat<0 || mat>=sizeof(g_matcolor)) return 0;
			ToolBrushSet(BRUSH_SHADER,SHADER_ALPHAREP);
			ToolBrushSet(BRUSH_COLOR, g_matcolor[mat]);
			return 1;
		}
		else
			return 0; // refuse objects
	}
	if(g_view==4) // material density view
	{
		if(ToolBrushGet(BRUSH_TYPE)==0)
		{
			if((ToolBrushGet(BRUSH_DRAW) & 2) == 0) return 0;
			mat = ToolBrushGet(BRUSH_MATERIAL);
			if(mat<0 || mat>=sizeof(g_matdensity)) return 0;
			matd = g_matdensity[mat];
			if(matd<0 || matd>=sizeof(g_matdensitycolor)) return 0;
			ToolBrushSet(BRUSH_SHADER,SHADER_ALPHAREP);
			ToolBrushSet(BRUSH_COLOR, g_matdensitycolor[matd]);
			return 1;
		}
		else
			return 0; // refuse objects

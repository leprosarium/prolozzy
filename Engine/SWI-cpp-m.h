...
public:
  ...
  PlFunctor(const char *name, int arity = 0)
  { functor = PL_new_functor(PL_new_atom(name), arity);
  }
  operator functor_t() const
  { return functor;
  }
};
...
public:
  ...
  friend bool operator <(const PlAtom &l, const PlAtom &r) 
...
public:
  ...
  PlQuery(module_t m, predicate_t p,const PlTermv &av)
  {
...
__inline
PlCompound::PlCompound(const PlFunctor &functor, const PlTermv &args) : PlTerm()
{ if ( !PL_cons_functor_v(ref, functor, args.a0) )
    throw PlResourceError();
}

__inline
PlCompound::PlCompound(functor_t functor, const PlTermv &args) : PlTerm()
{ if ( !PL_cons_functor_v(ref, functor, args.a0) )
    throw PlResourceError();
}
		 /*******************************

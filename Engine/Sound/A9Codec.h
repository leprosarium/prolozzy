...
public:
					a9Codec();
virtual				~a9Codec()								{};
virtual	int			Open( const std::string & name )		{ return A9_FAIL; }
...
// INTERFACE
///////////////////////////////////////////////////////////////////////////////////////////////////
		int			A9_CodecFind( const std::string & filename );						// find a codec that can play this file
...
inline	int			A9_CodecOpen( A9CODEC codec, const std::string & name )		{ 	assert(codec); return codec->Open(name);  }
...
inline	a9Info*		A9_CodecGetInfo( A9CODEC codec )							{  	assert(codec); return &(codec->m_info);  }
		...
		int			A9_CodecDecodeToWave( const std::string & filename, byte* &buffer, int& size );	// decode into a wave file buffer; user must free the buffer when done; needed codec library must be initialized; returns a9 error

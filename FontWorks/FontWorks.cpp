...
inline void	ItemSetText( int id, const std::wstring &  text )
{
	SetDlgItemTextW(g_hwnd,id,text.c_str());
}

inline std::wstring ItemGetText( int id ) 
{ 
	static wchar_t sz[320];
	GetDlgItemTextW(g_hwnd,id,sz,320);
	sz[319]=0;
	return sz;
}
...
wchar_t	g_character_list[65536];	// characters list
...
//////////////////////////////////////////////////////////////////////////////////////////////////
BOOL LoadTexture(const std::wstring & filepath, byte* &buffer, int &w, int &h, int &bpp )
{
	buffer=NULL;
	w=0,h=0,bpp=0;
...
BOOL OnTgaDrop(const std::wstring &  filepath )
{
	byte* buffer=NULL;
	int w=0,h=0,bpp=0;
...
BOOL OnTgaBrowse()
{
	...
	ofn.lpstrFilter			= L"*.tga\0*.tga\0\0";
	...
	ofn.lpstrTitle			= L"Select TGA font file.";
	ofn.Flags				= flags;
...
//@	logfont.lfHeight = ItemGetNumber(IDC_PAINT_H);
	...
	wcscpy(logfont.lfFaceName, ItemGetText(IDC_PAINT_FONT).c_str());
...
void OnInitDialog( HWND hwnd )
{
	...
	// set defaults
	ItemSetText(	IDC_TEXTURE_FILE,	L"font.tga" );
	...
	ItemSetText(	IDC_PAINT_FONT,		L"Arial" );
	...
	ItemSetText(	IDC_CHARACTER_LIST,	L"ABCDEFGHIJKLMNOPQRSTUVWXYZ" );
...
void OnFontConvert()
{
	...
	ofn.hwndOwner = g_hwnd;
	ofn.hInstance = g_hinstance;
	ofn.lpstrFilter = L"*.fnt\0*.fnt\0\0";
	ofn.lpstrCustomFilter = NULL;
	ofn.nMaxCustFilter = 0;
	ofn.nFilterIndex = 0;
	ofn.lpstrFile = path; // in+out
	...
	ofn.nMaxFileTitle = 0;
	ofn.lpstrInitialDir = NULL;
	ofn.lpstrTitle = L"Select FNT font file.";
	ofn.Flags = flags;
	ofn.nFileExtension = 0; //out
	ofn.lpstrDefExt = L"fnt";
	...
	switch(uMsg)
	{
		...
		case WM_COMMAND:
		{
			switch(LOWORD(wParam)) // by ID
			{
				...
				case IDC_FONT_CONVERT:		OnFontConvert(); return TRUE;
		...
		case WM_DROPFILES:
		{
			...
			if(file_path2ext(filepath) == L"tga")
				OnTgaDrop(filepath);
			else
...
// PAINT
//////////////////////////////////////////////////////////////////////////////////////////////////
BOOL PaintChar( HDC hdc1, HDC hdc2, RECT& rect, wchar_t chr, HFONT font, LOGFONTW& logfont, dword ink, dword paper, BOOL cell )
{
	int x,y,chrw;
	HFONT fontold;
	...
	GetCharWidth32W( hdc1, chr, chr, &chrw );
	...
	// LOG
	if (iswgraph(static_cast<wint_t>(chr)))
		o << chr;
	else
		o << "#" << std::setw(6) << std::setfill(L'0') << static_cast<int>(chr);
	o << " ";
	if (chr && chr % 16 == 0)
		o << L"\r\n";
	LOG(o.str());
...
BOOL Paint( HDC hdc1, HDC hdc2, HFONT font, LOGFONTW& logfont )
{
	int i,j;
	...
	{
		wchar_t chr = g_character_list[i];
		int cx = cp % mapw;
		int cy = cp / mapw;
		int x = mapx + celw * cx;
		int y = mapy + celh * cy;
...
BOOL ReadCharacters()
{
	...
	if(charset==0)
	{
		for(auto c: ItemGetText(IDC_CHARACTER_LIST))
			g_character_list[g_character_count++] = c;
	}
	else
	{
		int charbits = 0;
...
void MakePostProcess()
{
	...
	std::wostringstream o;
	o << L"Max char width: " << chrw << L"\r\n";
	if (cnt != g_character_count)
		o << L"Some characters where fount empty (" << cnt << "<" << g_character_count << ").";
	else
		o << "All characters are valid (" << g_character_count << ")";
	o << L"\r\n";
	LOG(o.str());
}
...
BOOL MakeProcess()
{
	...
	if(usealpha && g_tgabpp!=32)
	{
		MSGERROR("Texture has no alpha.\nCan't use alpha for best fit.");
	...
	memset(g_font.m_chars,0,sizeof(tFontChr)*65536);
	...
	// save font
	std::wstring szfile = ItemGetText(IDC_TEXTURE_FILE);
	std::wstring::size_type p = szfile.rfind('.');
	if (p != std::wstring::npos)
		szfile = szfile.substr(0, p);
	szfile += L".fnt";
	if (!SaveFont(szfile, g_font))
	{
